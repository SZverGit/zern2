          ...
          <v-card-text>
            ...
            <table>
              ...
              <tr>
                ...
                <td  class="px-12"><h2><input type="number" v-model="row.amount"></input></h2></td>
              </tr>
          ...
          <v-card-text>
            <table>
              ...
              <tr v-show="row.amount > row.location.reduce((acc,cur)=>acc+=cur.amount,0)">
               ...
               <td class="px-3">
                 <!--<v-select autocomplete item-text="name" v-model="newLocation.address" :items="$store.state.addressList">-->
                  <!--<v-chip class="px-3" slot="no-data">нет такого адреса!</v-chip>-->
                <!--</v-select>-->
                 <v-autocomplete autocomplete item-text="name" :items="$store.state.addressList" v-model="newLocation.address">
                   <v-chip class="px-3" slot="no-data">нет такого адреса!</v-chip>
                 </v-autocomplete>
  ...
  export default {
    ...
    methods:{
      addLocation(row){
        ...
        if (row.amount < row.location.reduce((acc,cur)=>acc+=cur.amount,0)+Number.parseInt(this.newLocation.amount)) {
            this.error.push('общее кол-во не бьется');
            this.$emit('Message', {msg:'Обновление прошло успешно, обновлено ',type:'success'})
            return
        }
