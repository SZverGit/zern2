<template>
  <v-container fluid>
    ...
    <v-toolbar dark class="fixed-tabs-bar no-print">
      <v-menu
      ...
      >
        <template v-slot:activator="{ on }">
          <v-btn @space.stop id="button" v-on="on" small color="success">
        ...
        <v-card>
          <v-card-text>
            <v-autocomplete item-text="name" return-object @change="menu.error=''" v-model="newSupply.supply" :items="Object.values($store.state.supplyList)" label="наименование">
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn text @click="menu.on = false">Закрыть</v-btn>
            <v-btn color="primary" text @click="addSupply">Добавить</v-btn>
          </v-card-actions>
        </v-card>
      </v-menu>
  ...
  export default {
    ...
    methods:{
            ...
            downloadExcel(){
                saveAs(toExcel(
                    this.operation.rows)
                    ,'Заявка '+this.operation.name +'.xlsx')
            },
        rowChanged(ra){
